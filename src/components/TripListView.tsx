...
export default function TripListView({ trips, inventory, profile, isGuest, customLists, allEssentials, onAddTrip, onDeleteTrip, onSelectTrip }: Props) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) {
      console.warn('Trip image upload cancelled: No file selected');
      return;
    }

    console.log('Starting trip image upload process...', { fileName: file.name, fileSize: file.size });

    try {
      const compressedFile = await compressImage(file);
      console.log('Trip image compressed successfully', { originalSize: file.size, compressedSize: compressedFile.size });
      
      const reader = new FileReader();
      reader.onload = (event) => {
        const dataURL = event.target?.result as string;
        setNewTripImage(dataURL);
        toast.success(t('trips.imageUploaded', 'Image uploaded successfully'));
      };
      reader.onerror = () => {
        toast.error(t('trips.imageUploadError', 'Failed to upload image'));
      };
  ...
  const handleRemoveNewTripImage = async () => {
    setNewTripImage(undefined);
  };

  const handleCancelCreate = async () => {
    await handleRemoveNewTripImage();
    setIsCreating(false);
    setNewTripName('');
              ...
              <div className="flex items-center gap-4">
                {newTripImage && (
                  <div className="relative shrink-0">
                    <img src={newTripImage} alt="Preview" className="w-16 h-16 rounded-xl object-cover border border-stone-200 dark:border-stone-700" referrerPolicy="no-referrer" />
                    <button
                      type="button"
                      onClick={handleRemoveNewTripImage}
      ...
      <div className="grid grid-cols-1 gap-8">
        {upcomingTrips.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {upcomingTrips.map(trip => <TripCard key={trip.id} trip={trip} onSelectTrip={onSelectTrip} onDelete={onDeleteTrip} profile={profile || undefined} />)}
            </div>
          </div>
        )}
        ...
        {sharedTrips.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {sharedTrips.map(trip => <TripCard key={trip.id} trip={trip} onSelectTrip={onSelectTrip} onDelete={onDeleteTrip} profile={profile || undefined} />)}
            </div>
          </div>
        )}
        ...
        {pastTrips.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-75 grayscale-[0.2]">
              {pastTrips.map(trip => <TripCard key={trip.id} trip={trip} onSelectTrip={onSelectTrip} onDelete={onDeleteTrip} profile={profile || undefined} />)}
            </div>
          </div>
        )}
