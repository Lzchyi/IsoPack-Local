...
export default function TripDetailView({ activeUserId, trip, inventory, profile, customLists, allEssentials, updateTrip, onDeleteTrip, onBack, onAddItem }: Props) {
  ...
  const presetLists = useMemo(() => Object.entries(SUGGESTED_ITEMS)
    .filter(([key]) => key !== 'All')
    .map(([key, items]) => [
      key, 
      key === 'All Essentials' ? allEssentials.map(item => ({ name: item.name, category: item.category })) : items
    ] as [string, { name: string, category: Category }[]]), [allEssentials]);
  ...
  const [coverImage, setCoverImage] = useState(trip.imageUrl);

  // Custom item state
  const [customName, setCustomName] = useState('');
  const [customCategory, setCustomCategory] = useState<Category>(Category.Essentials);
  ...
  const toggleItemPacked = (itemId: string) => {
    console.log('toggleItemPacked called for:', itemId);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        // Save this string directly to your trip object in IndexedDB
        await db.trips.update(trip.id, { imageUrl: base64String });
        updateTrip({ ...trip, imageUrl: base64String });
                  ...
                  <div>
                    {searchQuery.trim() === '' ? (
                      ...
                      <div className="space-y-2 h-[400px]">
                        {(() => {
                          const allResults = [...availableMustBring, ...availableGear, ...availableSuggestions];
                          const searchResults = [];
                            ...
                            <Virtuoso
                              ...
                              itemContent={(index, item) => (
                                <div className="flex items-center justify-between p-4 mb-2 rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 hover:border-emerald-500/30 hover:shadow-md transition-all group">
                                  <div>
                                    <p className="font-medium text-stone-900 dark:text-white">{String(t(`item.${item.name}`, item.name))}</p>
